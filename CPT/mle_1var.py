...
param_arr = np.array([.8, .9, 1., 1.1, 1.2])
sim_arr = param_arr
...
#running simulations
if run_sim:
    for i, val in enumerate(sim_arr):
        print(f'running simulation for {parameter} {val}')
        os.system('date +%c')
        change_parameter(parameters, val, multiplier=unit_multiplier)
        if parameter == 'emit':
            change_parameter(np.array(['sigma_x', 'sigma_y']), ss_arr[i], multiplier=unit_multiplier)
...
if plot_type == 'spec' or plot_type == 'both':
    ...
    #spec_exp, energies, _, residuals = get_spectrum_function(output_exp, rad_exp)
