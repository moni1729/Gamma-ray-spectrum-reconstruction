...
def get_dd_function(general_output_filename, radiation_output_filename):
    output_dict = {}
    ...
    return(dd2_wit.tolist(), energies_midpoint.tolist(), phi_xs_midpoint.tolist())
...
num_ext = '' # include the _
data_file = f'training/{type}_data_emit_dd{num_ext}.csv'
param_file = f'training/{type}_emit{num_ext}.csv'
ref_file = f'training/{type}_data_emit{num_ext}.csv'
num_particles = 64
ext = '_e64p64_train'
folder_number = 4
plot_type = 'spec'
parameters = np.array(['emit_n_x', 'emit_n_x'])
parameter = 'emit'
param_units = 'micron'
...
for i, val in enumerate(param_list):
    ...
    dd, energies, phi_xs = get_dd_function(general_output_filename, radiation_output_filename)

    print(f'writing parameter value {i + 1}: {dd[0][0]}')
    ...
    if(i==0):
        ...
        print(f'shape of dd: {len(dd)}, {len(dd[0])}')
